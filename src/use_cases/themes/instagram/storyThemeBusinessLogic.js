...
export class StoryThemeBusinessLogic {
    ...
    _closeProductCardAtTouch(elem) {
        const card = elem.querySelector('.sitestories-story-product-card__card')
        const self = this
        let touchStartY = 0
        let touchDifferenceY = 0
        let currentPositionY = 0
        let positionMoveY = 0
        let swipe_distance_hide = 80
        ...
        const touchmoveHandler = (e) => {
            ...
            card.style.transform = `translateY(${positionMoveY}px)`
        ...
        card.addEventListener('touchcancel', touchcancelHanler, { passive: true })
    }
}